...
/** 燈光輔助線 */
function supportLight(shadowLight) {
...
/** 右到左移動整個畫布(順暢地從右到左) */
function moveCanvas() {
	const canvas = document.getElementById('usagi').childNodes[0];
...
// animateGaspTest();
setTimeout(() => run(), 0);
setTimeout(() => prone(), CanvasMoveTime * 1000 + 100);
setTimeout(() => walk(), CanvasMoveTime * 2000 + 200);
setTimeout(() => dance(), CanvasMoveTime * 3000 + 300);
setTimeout(() => run(), CanvasMoveTime * 4000 + 400);
